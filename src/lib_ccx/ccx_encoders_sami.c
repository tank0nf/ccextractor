...
int write_stringz_as_sami(char *string, struct encoder_ctx *context, LLONG ms_start, LLONG ms_end)
{
	...
	ret = write(fileno(context->out->fh), context->buffer, used);
	if (ret != used)
		return ret;
	...
	while (begin < unescaped + len)
	{
		...
		ret = write(fileno(context->out->fh), el, u);
		if (ret != u)
			goto end;

		ret = write(fileno(context->out->fh), context->encoded_br, context->encoded_br_length);
		...
		for (int i = sub->nb_data - 1; i >= 0; i--)
		{
			if (rect[i].ocr_text && *(rect[i].ocr_text))
			{
				if (context->prev_start != -1 || !(sub->flags & SUB_EOD_MARKER))
				{
					...
					write_wrapped(fileno(context->out->fh), buf, strlen(buf));
					if (i != 0)
						write_wrapped(fileno(context->out->fh), context->encoded_br, context->encoded_br_length);
...
int write_cc_buffer_as_sami(struct eia608_screen *data, struct encoder_ctx *context)
{
	...
	for (int i = 0; i < 15; i++)
	{
		if (data->row_used[i])
		{
			...
			write_wrapped(fileno(context->out->fh), context->subline, length);
			wrote_something = 1;
			if (i != 14)
				write_wrapped(fileno(context->out->fh), context->encoded_br, context->encoded_br_length);
			write_wrapped(fileno(context->out->fh), context->encoded_crlf, context->encoded_crlf_length);
